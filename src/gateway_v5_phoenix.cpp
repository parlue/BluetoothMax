// Frozen v5 snapshot of the multi-board gateway (main.cpp and friends),
// taken right after the Chessnut+Mephisto Phoenix status-checksum bug was
// found and fixed on real hardware: outgoing cable checksums were being
// computed over the odd-parity-encoded bytes instead of the plain content
// whenever the connected host used the encoded-checksum convention (as
// Phoenix always does) -- confirmed against Elfacun/Diablillo's own
// proven-working reference source, which always computes outgoing
// checksums over plain content. This silently corrupted roughly half of
// all single-square Chessnut status updates (a precise, hand-verified
// 16-of-32-piece mirror-symmetric pattern by piece letter) while leaving
// paired two-square updates (ordinary moves) unaffected, which is why it
// went unnoticed for so long. Fixed for every outgoing reply (Chessnut/
// Cynus status frames, and the V/X/R/W/l cable acks); Phoenix now
// correctly registers every single-square change, capture and castling on
// direct cable connection, confirmed by the user through a full real game.
// Also includes: BT-BT standalone mode's late-cable-detected auto-restart
// fallback (no longer needs a manual power cycle if the cable module boots
// slowly), the "Chessnut Air" masquerade name, and the local Chessnut
// "show missing pieces" LED aid. Built by the
// `esp32-c3-superminiV5_Phoenix` PlatformIO environment and never touched
// by any later work -- flash this to fall back to this exact,
// already-proven behavior. Self-contained: uses only the _v5 copies of
// board_driver/millennium_board/chessnut_board/cynus_board/chesslink_server/
// chessnut_server, never the live (non-_v5) ones, so ongoing work can never
// affect this build -- unlike the earlier V3/V4 snapshots, which never
// included their own copies of chesslink_server/chessnut_server since
// standalone BT-BT mode didn't exist yet when they were frozen.
